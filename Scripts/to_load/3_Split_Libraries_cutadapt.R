...
splitLibrariesByPrimers <- function(fnFs = list.files(path = "~/Documents/16S-ITS_trial", pattern = "R1_001.fastq.gz", full.names = TRUE), # Path for raw data
                                    fnRs = list.files(path = "~/Documents/16S-ITS_trial", pattern = "R2_001.fastq.gz", full.names = TRUE),
                                    ...
                                    split.ITS = T) # cutadapt path
{
  ...
  # 16S output files
  fnFs.16S <- file.path(path.16S, paste0(basename(
    tools::file_path_sans_ext(fnFs, 
                              compression = T)), 
    "_16S.fastq.gz"))
  fnRs.16S <- file.path(path.16S, paste0(basename(
    tools::file_path_sans_ext(fnRs, 
                              compression = T)), 
    "_16S.fastq.gz"))
  ...
  ## Final ITS
  fnFs.ITS <- file.path(path.ITS, paste0(basename(
    tools::file_path_sans_ext(fnFs, 
                              compression = T)), 
    "_ITS.fastq.gz"))
  fnRs.ITS <- file.path(path.ITS, paste0(basename(
    tools::file_path_sans_ext(fnRs, 
                              compression = T)), 
    "_ITS.fastq.gz"))
  ...
  for(i in seq_along(fnFs)) {
    ...
    system2("cutadapt", args = c(R1.flags, R2.flags, "-n", 2, "--cores=0 --discard-untrimmed --pair-filter=both",
  ...
  for(i in seq_along(fnFs)) {
    ...
    system2("cutadapt", args = c(R1.flags, R2.flags, "-n", 2, "--cores=0 --discard-untrimmed --pair-filter=both", # Read length cutoff removed for testing "--minimum-length 10" 
                               "-o", fnFs.ITS[i], "-p", fnRs.ITS[i], # output files
                               fnFs[i], fnRs[i])) # input files # You can put fn(F/R)s.ITS.wp[i] here, if you want to exclude 16S reads and get rid of possible overlap in libraries.
