GroupTest_mod <- function(species.table = NULL,
  ...
  for (i in grep(pattern = "_IncertaeSedis", x = colnames(taxa))) {
    colnames(taxa)[i] <- gsub(
      x = colnames(taxa)[i], pattern = "_IncertaeSedis", fixed = T,
  ...
  for (i in grep(pattern = "_uncultured", x = colnames(taxa))) {
    colnames(taxa)[i] <- gsub(
      x = colnames(taxa)[i], pattern = "_uncultured", fixed = T,
      replacement = paste("_", strsplit(x = colnames(taxa)[i], split = "_")[[1]][4], strsplit(x = colnames(taxa)[i], split = "_")[[1]][5], sep = "")
    )
  }
  ...
  if (ncol(taxa) > 0) {
    ...
    dataset <- data.frame(
      dataset[, c(1:ncol(meta), ncol(dataset))],
    ...
    group_test <- data.frame(array(dim = c(length(names(taxa)), length(c(
      "taxon", paste(levels(dataset[, group])[-1], "estimate", sep = "_"),
    ...
    if (length(subject.ID) != 0) {
      ...
      modeldata <- na.omit(dataset[, c(
        names(taxa), group, "G", paste(confounders, sep = ", "), "ID",
        "ReadCount"
      )[c(names(taxa), group, "G", paste(confounders, sep = ", "), "ID", "ReadCount") != ""]])
      ...
      for (i in names(colSums(modeldata[, names(taxa)] > 0)[colSums(modeldata[, names(taxa)] > 0) > 5])) {
        model[[i]] <- tryCatch(
          glmmADMB::glmmadmb(
            as.formula(paste(
              ...
              "+", "offset(log(ReadCount))"
            )),
            random = ~ 1 | ID, family = "nbinom",
            data = modeldata,
        ...
        if (summary(lm(tmp$res ~ tmp$fitted))$coef[2, 4] > 0.1 & summary(lm(tmp$resdev ~ tmp$fitted))$coef[2, 4] > 0.1 &
          ...
          if (summary(lm(tmp$res ~ tmp$fitted))$coef[2, 4] < 0.01 | summary(lm(tmp$resdev ~ tmp$fitted))$coef[2, 4] < 0.01) {
            if (abs(mean((modeldata[, i] + 1) / modeldata$ReadCount) - median((modeldata[, i] + 1) / modeldata$ReadCount)) / median((modeldata[, i] + 1) / modeldata$ReadCount) < 0.25) {
              model[[i]] <- tryCatch(
                nlme::lme(as.formula(paste("(", i, "/ReadCount)~", confounders_formula, "+ ", group)),
              ...
              model[[i]] <- tryCatch(
                nlme::lme(as.formula(paste("log(", i, "/ReadCount)~", confounders_formula, "+ ", group)),
                  data = modeldata2, weights = nlme::varExp(), random = ~ 1 | ID,
                  control = nlme::glsControl(maxIter = 1000)
                ),
                error = function(e) NULL
        ...
        if (length(model[[i]]) > 0) {
          ...
          for (j in levels(modeldata[, group])[-1]) {
            ...
            if (group_test[i, "model"] == "lme.formula") {
              group_test[i, paste(j, "estimate_FoldChange", sep = "_")] <- gtools::foldchange(
                (summary(model[[i]])$tTable[1, 1] + summary(model[[i]])$tTable[paste(group, j, sep = ""), 1]),
          ...
          if (length(confounders[confounders != ""]) > 0) {
            ...
            resids[, i] <- tryCatch(
              resid(update(model[[i]], as.formula(paste(".~. -", group, sep = ""))),
                type = "pearson"
              ),
              error = function(e) NA
            )
      ...
      if (nonzero) {
        ...
        for (i in names(colSums(modeldata[, names(taxa)] > 0)[colSums(modeldata[, names(taxa)] > 0) > 5])) {
          if (min(table(modeldata[modeldata[, i] > 0, group])) > 2) {
            model[[paste(i, "nonzero")]] <- tryCatch(
              glmmADMB::glmmadmb(
                as.formula(paste(
                  "round(",
                  i, ")~", confounders_formula, "+ ", group,
            ...
            if (length(model[[paste(i, "nonzero")]]) > 0) {
              ...
              for (j in levels(modeldata[, group])[-1]) {
                ...
                if (group_test_nonzero[i, "model"] == "lme.formula") {
                  group_test_nonzero[i, paste(j, "estimate_FoldChange", sep = "_")] <- gtools::foldchange(
                    (summary(model[[paste(i, "nonzero")]])$tTable[1, 1] + summary(model[[paste(i, "nonzero")]])$tTable[paste(group, j, sep = ""), 1]),
            ...
            if (length(confounders[confounders != ""]) > 0) {
              residsNonzero[modeldata[, i] > 0, i] <- tryCatch(
                resid(update(model[[paste(i, "nonzero")]], as.formula(paste(".~. -", group, sep = ""))),
                  type = "pearson"
                ),
                error = function(e) NA
              )
      ...
      modeldata <- na.omit(dataset[, c(
        names(taxa), group, "G", paste(confounders, sep = ", "),
        "ReadCount"
      )[c(names(taxa), group, "G", paste(confounders, sep = ", "), "ReadCount") != ""]])
      modeldata[, group] <- modeldata[, group][drop = T]
      resids <- modeldata
      modeldata2 <- modeldata
      modeldata2[, names(taxa)] <- modeldata2[, names(taxa)] + 1
      ...
      for (i in names(colSums(modeldata[, names(taxa)] > 0)[colSums(modeldata[, names(taxa)] > 0) > 5])) {
        model[[i]] <- tryCatch(
          MASS::glm.nb(formula(paste("round(", i, ")~", confounders_formula, "+ ", group, "+", "offset(log(ReadCount))")),
        ...
        model_test <- tryCatch(
          MASS::glm.nb(formula(paste("round(", i, ")~", confounders_formula, "+", "offset(log(ReadCount))")),
            data = modeldata, control = glm.control(maxit = 1000, epsilon = 1e-12)
          ),
          error = function(e) NULL
        )
        ...
        if (length(model[[i]]) == 0 | is.null(model_test)) {
          model[[i]] <- tryCatch(
            MASS::glm.nb(formula(paste("round(", i, "+1)~", confounders_formula, "+ ", group, "+", "offset(log(ReadCount))")),
              data = modeldata, control = glm.control(maxit = 1000, epsilon = 1e-12)
            ),
            error = function(e) NULL
          )
        ...
        if (length(model[[i]]) == 0) {
          model[[i]] <- tryCatch(
            stats::glm(formula(paste("round(", i, ")~", confounders_formula, "+ ", group, "+", "offset(log(ReadCount))")),
              family = poisson,
              data = modeldata, control = glm.control(maxit = 1000, epsilon = 1e-12)
            ),
            error = function(e) NULL
        ...
        if (length(model[[i]]) == 0) {
          if (abs(mean(modeldata[, i] / modeldata$ReadCount) - median(modeldata[, i] / modeldata$ReadCount)) / median(modeldata[, i] / modeldata$ReadCount) < 0.25) {
            model[[i]] <- tryCatch(
              stats::lm(formula(paste("(", i, "+1)/ReadCount~", confounders_formula, "+ ", group)),
                data = modeldata
              ),
              error = function(e) NULL
            )
            ...
            model[[i]] <- tryCatch(
              stats::lm(formula(paste("log(", i, "+1)/ReadCount~", confounders_formula, "+ ", group)),
                data = modeldata
              ),
              error = function(e) NULL
            )
        ...
        if (length(model[[i]]) > 0) {
          ...
          if (summary(stats::lm(tmp$res ~ tmp$fitted))$coef[2, 4] > 0.1 & summary(stats::lm(tmp$resdev ~ tmp$fitted))$coef[2, 4] > 0.1 &
            ...
            if (summary(lm(tmp$res ~ tmp$fitted))$coef[2, 4] < 0.01 | summary(lm(tmp$resdev ~ tmp$fitted))$coef[2, 4] < 0.01) {
              if (abs(mean((modeldata[, i] + 1) / modeldata$ReadCount) - median((modeldata[, i] + 1) / modeldata$ReadCount)) / median((modeldata[, i] + 1) / modeldata$ReadCount) < 0.25) {
                model[[i]] <- tryCatch(
                  nlme::gls(formula(paste("(", i, "/ReadCount)~", confounders_formula, "+ ", group)),
                ...
                model[[i]] <- tryCatch(
                  nlme::gls(formula(paste("log(", i, "/ReadCount)~", confounders_formula, "+ ", group)),
            ...
            for (j in levels(modeldata[, group])[-1]) {
              if (group_test[i, "model"] == "MASS::glm.nb" | group_test[i, "model"] == "glm") {
                ...
                if (length(confounders[confounders != ""]) > 0) {
                  ...
                  resids[, i] <- tryCatch(
                    resid(update(model[[i]], as.formula(paste(".~. -", group, sep = "")),
                      init.theta = 5, control = glm.control(epsilon = 1e-12, maxit = 2500, trace = T)
                    ), type = "pearson"),
                    error = function(e) NA
                  )
                  ...
                  if (is.na(sum(resids[, i]))) {
                    ...
                    resids[, i] <- tryCatch(
                      resid(update(model[[i]], as.formula(paste(".~. -", group, sep = "")),
                        control = glm.control(epsilon = 1e-20, maxit = 1000, trace = T)
                      ), type = "pearson"),
                      error = function(e) NA
                    )
                  }
                  if (is.na(sum(resids[, i]))) {
                    ...
                    resids[, i] <- tryCatch(
                      resid(update(model[[i]], as.formula(paste(".~. -", group, sep = "")),
                        init.theta = 10, control = glm.control(epsilon = 1e-20, maxit = 2500, trace = T)
                      ), type = "pearson"),
                      error = function(e) NA
                    )
                  }
              ...
              if (group_test[i, "model"] == "nlme::gls") {
                group_test[i, paste(j, "estimate_FoldChange", sep = "_")] <- gtools::foldchange(
                  mean(fitted(model[[i]])[modeldata$G == j]),
                  mean(fitted(model[[i]])[modeldata$G == compare.to])
                ) # (summary(model[[i]])$tTable[1,1]+summary(model[[i]])$tTable[paste(group,j,sep=""),1])/summary(model[[i]])$tTable[1,1]
                if (length(confounders[confounders != ""]) > 0) {
                  assign("i", paste(i), envir = .GlobalEnv)
              ...
              if (group_test[i, "model"] == "log nlme::gls") {
                group_test[i, paste(j, "estimate_FoldChange", sep = "_")] <- gtools::foldchange(
                  mean(exp(fitted(model[[i]])[modeldata$G == j])),
                  mean(exp(fitted(model[[i]])[modeldata$G == compare.to]))
                ) # exp(summary(model[[i]])$tTable[1,1]+(summary(model[[i]])$tTable[paste(group,j,sep=""),1]))/exp(summary(model[[i]])$tTable[1,1])
              ...
              if (group_test[i, "model"] == "lm") {
                group_test[i, paste(j, "estimate_FoldChange", sep = "_")] <- gtools::foldchange(
                  mean(fitted(model[[i]])[modeldata$G == j]),
                  mean(fitted(model[[i]])[modeldata$G == compare.to])
                ) # (summary(model[[i]])$coef[1,1]+summary(model[[i]])$coef[paste(group,j,sep=""),1])/summary(model[[i]])$coef[1,1]
                if (length(confounders[confounders != ""]) > 0) {
                  assign("i", paste(i), envir = .GlobalEnv)
              ...
              if (group_test[i, "model"] == "log lm") {
                ...
                group_test[i, paste(j, "estimate_FoldChange", sep = "_")] <- gtools::foldchange(
                  mean(exp(fitted(model[[i]])[modeldata$G == j])),
                  mean(exp(fitted(model[[i]])[modeldata$G == compare.to]))
                ) # (exp(summary(model[[i]])$coef[1,1]+summary(model[[i]])$coef[paste(group,j,sep=""),1]))/exp(summary(model[[i]])$coef[1,1])
                if (length(confounders[confounders != ""]) > 0) {
                  assign("i", paste(i), envir = .GlobalEnv)
      ...
      if (nonzero) {
        ...
        for (i in names(colSums(modeldata[, names(taxa)] > 0)[colSums(modeldata[, names(taxa)] > 0) > 5])) {
          if (min(table(modeldata[modeldata[, i] > 0, group])) > 3) {
            ...
            model[[paste(i, "nonzero")]] <- tryCatch(
              MASS::glm.nb(as.formula(paste("round(", i, ")~", confounders_formula2, "+ ", group, "+", "offset(log(ReadCount))")),
            ...
            if (length(model[[paste(i, "nonzero")]]) == 0) {
              model[[paste(i, "nonzero")]] <- tryCatch(
                MASS::glm.nb(as.formula(paste("round(", i, "+1)~", confounders_formula2, "+ ", group, "+", "offset(log(ReadCount))")),
                  data = modeldata[modeldata[, i] > 0, ], control = glm.control(maxit = 1000)
                ),
                error = function(e) NULL
              )
            ...
            if (length(model[[paste(i, "nonzero")]]) == 0) {
              model[[i]] <- tryCatch(
                glm(formula(paste("round(", i, ")~", confounders_formula, "+ ", group, "+", "offset(log(ReadCount))")),
                  family = poisson,
                  data = modeldata[modeldata[, i] > 0, ]
                ),
                error = function(e) NULL
            ...
            if (length(model[[paste(i, "nonzero")]]) == 0) {
              if (abs(mean(modeldata[modeldata[, i] > 0, i] / modeldata$ReadCount[modeldata[, i] > 0]) - median(modeldata[modeldata[, i] > 0, i] / modeldata$ReadCount[modeldata[, i] > 0])) / median(modeldata[modeldata[, i] > 0, i] / modeldata$ReadCount[modeldata[, i] > 0]) < 1) {
                model[[paste(i, "nonzero")]] <- tryCatch(
                  lm(as.formula(paste("(", i, "+1)/ReadCount~", confounders_formula2, "+ ", group)),
                    data = modeldata[modeldata[, i] > 0, ]
                  ),
                  error = function(e) NULL
                )
                ...
                model[[paste(i, "nonzero")]] <- tryCatch(
                  lm(as.formula(paste("log(", i, "+1)/ReadCount~", confounders_formula2, "+ ", group)),
                    data = modeldata[modeldata[, i] > 0, ]
                  ),
                  error = function(e) NULL
                )
            ...
            if (summary(lm(tmp$res ~ tmp$fitted))$coef[2, 4] != "NaN" & summary(lm(tmp$resdev ~ tmp$fitted))$coef[2, 4] != "NaN" &
              ...
              if (summary(lm(tmp$res ~ tmp$fitted))$coef[2, 4] < 0.01 | summary(lm(tmp$resdev ~ tmp$fitted))$coef[2, 4] < 0.01) {
                if (abs(mean(modeldata[modeldata[, i] > 0, i] / modeldata$ReadCount[modeldata[, i] > 0]) - median(modeldata[modeldata[, i] > 0, i] / modeldata$ReadCount[modeldata[, i] > 0])) / median(modeldata[modeldata[, i] > 0, i] / modeldata$ReadCount[modeldata[, i] > 0]) < 1) {
                  model[[paste(i, "nonzero")]] <- tryCatch(
                    nlme::gls(as.formula(paste("(", i, "/ReadCount)~", confounders_formula2, "+ ", group)),
                  ...
                  model[[paste(i, "nonzero")]] <- tryCatch(
                    nlme::gls(as.formula(paste("log(", i, "/ReadCount)~", confounders_formula2, "+ ", group)),
                      data = modeldata[modeldata[, i] > 0, ], weights = nlme::varExp(), control = nlme::glsControl(maxIter = 1000)
                    ),
                    error = function(e) NULL
                  )
            ...
            if (length(model[[paste(i, "nonzero")]]) > 0) {
              ...
              for (j in levels(modeldata[, group])[-1]) {
                ...
                if (group_test_nonzero[i, "model"] == "nlme::gls") {
                  group_test_nonzero[i, paste(j, "estimate_FoldChange", sep = "_")] <- gtools::foldchange(
                    mean(fitted(model[[paste(i, "nonzero")]])[modeldata$G == j & modeldata[, i] > 0]),
                    mean(fitted(model[[paste(i, "nonzero")]])[modeldata$G == compare.to & modeldata[, i] > 0])
                  ) # (summary(model[[paste(i,"nonzero")]])$tTable[1,1]+summary(model[[paste(i,"nonzero")]])$tTable[paste(group,j,sep=""),1])/summary(model[[paste(i,"nonzero")]])$tTable[1,1]
                  if (length(confounders[confounders != ""]) > 0) {
                    assign("i", paste(i), envir = .GlobalEnv)
                ...
                if (group_test_nonzero[i, "model"] == "log nlme::gls") {
                  group_test_nonzero[i, paste(j, "estimate_FoldChange", sep = "_")] <- gtools::foldchange(
                    mean(exp(fitted(model[[paste(i, "nonzero")]])[modeldata$G == j & modeldata[, i] > 0])),
                    mean(exp(fitted(model[[paste(i, "nonzero")]])[modeldata$G == compare.to & modeldata[, i] > 0]))
                  ) # exp(summary(model[[paste(i,"nonzero")]])$tTable[1,1]+(summary(model[[paste(i,"nonzero")]])$tTable[paste(group,j,sep=""),1]))/exp(summary(model[[paste(i,"nonzero")]])$tTable[1,1])
                  if (length(confounders[confounders != ""]) > 0) {
                    assign("i", paste(i), envir = .GlobalEnv)
                ...
                if (group_test_nonzero[i, "model"] == "lm") {
                  group_test_nonzero[i, paste(j, "estimate_FoldChange", sep = "_")] <- gtools::foldchange(
                    mean(fitted(model[[paste(i, "nonzero")]])[modeldata$G == j & modeldata[, i] > 0]),
                    mean(fitted(model[[paste(i, "nonzero")]])[modeldata$G == compare.to & modeldata[, i] > 0])
                  ) # (summary(model[[paste(i,"nonzero")]])$coef[1,1]+summary(model[[paste(i,"nonzero")]])$coef[paste(group,j,sep=""),1])/summary(model[[paste(i,"nonzero")]])$coef[1,1]
                  if (length(confounders[confounders != ""]) > 0) {
                    assign("i", paste(i), envir = .GlobalEnv)
                ...
                if (group_test_nonzero[i, "model"] == "log lm") {
                  group_test_nonzero[i, paste(j, "estimate_FoldChange", sep = "_")] <- gtools::foldchange(
                    mean(exp(fitted(model[[paste(i, "nonzero")]])[modeldata$G == j & modeldata[, i] > 0])),
                    mean(exp(fitted(model[[paste(i, "nonzero")]])[modeldata$G == compare.to & modeldata[, i] > 0]))
                  ) # (exp(summary(model[[paste(i,"nonzero")]])$coef[1,1]+summary(model[[paste(i,"nonzero")]])$coef[paste(group,j,sep=""),1]))/exp(summary(model[[paste(i,"nonzero")]])$coef[1,1])
                  if (length(confounders[confounders != ""]) > 0) {
                    assign("i", paste(i), envir = .GlobalEnv)
    ...
    if (nrow(na.omit(group_test)) > 0) {
      ...
      if (length(confounders[confounders != ""]) > 0) {
        if (nonzero) {
          ...
          resids2 <- data.frame(cbind(
            resids[, setdiff(colnames(resids), colnames(taxa))],
        ...
        for (i in levels(modeldata$G)[levels(modeldata$G) != compare.to]) {
          group_test[, paste("FoldChange", i, sep = "_")] <- gtools::foldchange(
            group_test[, paste("Mean", i, sep = "_")],
        ...
        if (nonzero) {
          ...
          for (i in levels(modeldata$G)[levels(modeldata$G) != compare.to]) {
            group_test_nonzero[, paste("FoldChange", i, sep = "_")] <- gtools::foldchange(
              group_test_nonzero[, paste("Mean", i, sep = "_")],
      ...
      # odds ratios ad confidence intervals
      odds_CI <- model %>%
        purrr::map(
          ~ try(exp(cbind(stats::coef(.x), stats::confint(.x))))) %>%
        purrr::keep(., ~ is.matrix(.)) %>% 
        purrr::imap(
            ~ data.frame(.x) %>%
            purrr::set_names("OR", "CI_2.5", "CI_97.5") %>%
            dplyr::filter(!rownames(.) == "(Intercept)") %>%
            `rownames<-`(., c(paste(.y, rownames(.), sep = "_")))
        ) %>%
        dplyr::bind_rows() %>% 
      ...
      writexl::write_xlsx(group_test,
        path =
          here::here(
            dir_for_res, "tables",
            paste("GroupTest_", xl_titl, "_",
              group, "_", compare.to, "_",
              select.by, "_", select,
              "_prev_", min.prevalence * 100, "_abd_",
              min.abundance * 100, ".xlsx",
              sep = ""
            )
          )
      )
      ...
      if (length(confounders[confounders != ""]) > 0) {
        writexl::write_xlsx(resids,
          path =
            here::here(
              dir_for_res, "tables",
              paste("GroupTest_residuals_", xl_titl, "_",
                group, "_",
      ...
      writexl::write_xlsx(odds_CI,
        path =
          here::here(
            dir_for_res, "tables",
            paste("GroupTest_Odds_CI_", xl_titl, "_",
              group, "_", compare.to, "_",
              select.by, "_", select,
              "_prev_", min.prevalence * 100, "_abd_",
              min.abundance * 100, ".xlsx",
        ...
        if (pdf) {
          pdf(
            ...
            file = here::here(
              ...
              paste("GroupTest_", group, "_", compare.to, "_", select.by,
                "_", select,
                "_prev_", min.prevalence * 100, "_abd_",
                min.abundance * 100, "_Boxplot.pdf",
                sep = ""
              )
            )
          )
          ...
          for (i in sig[order(sig)]) {
            ...
            boxplot(dataset2[, i] ~ dataset2[, "G"],
              ...
              outline = F,
              col = c(
                "#E41A1C", "orange", "#377EB8", "skyblue", "#4DAF4A", "#984EA3", "#FF7F00", "#FFFF33",
          ...
          pdf(
            ...
            file = here::here(
              ...
              paste("GroupTest_", group, "_", compare.to, "_", select.by, "_",
                select,
                "_prev_", min.prevalence * 100, "_abd_",
                min.abundance * 100, "_Beanplot.pdf",
                sep = ""
              )
            )
          )
